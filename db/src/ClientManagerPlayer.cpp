...
void CClientManager::RESULT_AFFECT_LOAD(CPeer* peer, MYSQL_RES* pRes, DWORD dwHandle, DWORD dwClientPID)
{
	...
	{
		TPacketAffectElement& element = elements[i];
		row = mysql_fetch_row(pRes);

		if (dwPID == 0)
			str_to_number(dwPID, row[0]);

		str_to_number(element.dwType, row[1]);
		str_to_number(element.bApplyOn, row[2]);
		str_to_number(element.lApplyValue, row[3]);
		str_to_number(element.dwFlag, row[4]);
		str_to_number(element.lDuration, row[5]);
		str_to_number(element.lSPCost, row[6]);
	}

	sys_log(0, "AFFECT_LOAD: Loaded %u affects for PID %u", elements.size(), dwPID);

	DWORD dwCount = elements.size();

	peer->EncodeHeader(HEADER_DG_AFFECT_LOAD, dwHandle, sizeof(dwPID) + sizeof(dwCount) + sizeof(TPacketAffectElement) * dwCount);
	peer->Encode(&dwPID, sizeof(dwPID));
	peer->Encode(&dwCount, sizeof(dwCount));
