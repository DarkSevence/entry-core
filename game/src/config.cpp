...
void config_init(const string& st_localeServiceName)
{
	...
	for (int i = 0; i < 2; ++i) {
	    fprintf(stderr, "db_host[%d]: %s\n", i, db_host[i]);
	    fprintf(stderr, "db_user[%d]: %s\n", i, db_user[i]);
	    fprintf(stderr, "db_pwd[%d]: %s\n", i, db_pwd[i]);
	    fprintf(stderr, "db_db[%d]: %s\n", i, db_db[i]);
	}	
	...
	*log_db = '\0';
	...
	fprintf(stderr, "Tetst5");

	//처리가 끝났으니 파일을 닫자.
	fclose(fpOnlyForDB);

	fprintf(stderr, "Tetst6");
