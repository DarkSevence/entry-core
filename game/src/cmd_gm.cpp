...
ACMD(do_user)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "[LS;459]", func.count);
}
...
ACMD(do_affect_remove)
{
	...
	if (!*arg1 || !*arg2)
	{
		...
		while (it != cont.end())
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "%4d %5d %5d %8d %u", pkAff->dwType, pkAff->bApplyOn, pkAff->lApplyValue, pkAff->lDuration, pkAff->dwFlag);
		}
		return;
	}
