...
bool CHARACTER::AddAffect(DWORD dwType, BYTE bApplyOn, long lApplyValue, DWORD dwFlag, long lDuration, long lSPCost, bool bOverride, bool IsCube)
{
    ...
    if (!CheckSkillAffect(dwType, lApplyValue)) 
	{
        return false;
    }
...
bool CHARACTER::CheckSkillAffect(DWORD dwType, long lApplyValue) 
{
	switch (dwType) 
	{
		case SKILL_HOSIN:
		...
		case SKILL_JEUNGRYEOK:
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("SKILL_%s_LOW_YOUR_%ld%%_USE_%ld%%"), CSkillManager::instance().Get(dwType)->szName, lApplyValue, pkAffect->lApplyValue);
				return false;
			}
		}
		break;
...
bool CHARACTER::RemoveFirstActiveAffect(uint32_t dwType)
{
	CAffect* pkAff = FindAffect(dwType);
